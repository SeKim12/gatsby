...
class Chromosome(ABC):
    ...
    @abstractmethod
    def interweave1d(
        self, start_1, end_1, other: "Chromosome", start_2, end_2
...
class Gatsby:
    ...
    def __init__(
        ...
        crossover_fn: Callable[
            [COP, Chromosome, Chromosome, float], Tuple[Chromosome, Chromosome]
        ],
        ...
        self._tabu_fn: Callable[[Chromosome], Chromosome] = (
            self._flip_tabu if which_tabu == "flip" else self._swap_tabu
        )
        ...
        self.best_chromosome: Optional[
            Chromosome
        ] = None  # best chromosome across ALL generations
    ...
    def _handle_optimal_converge(self, gen_start: float, cr: Chromosome):
        """Early termination method for when maximum fitness is reached mid-generation.

        Args:
            gen_start: The starting time of the current generation in progress.
            cr: The maximum fitness chromosome.

        """
        self.best_chromosome = cr
        self.fitness_hist.append(self._max_fitness)
        self.time_hist.append(time.perf_counter() - gen_start)
        self._converged = True
        self._print_info(
            f"\n**************************************"
            f"\nAchieved Max Fitness {self._max_fitness}"
            f"\n**************************************"
            f"\n",
            -1,
        )
    ...
    def run(self):
        ...
        for t in range(self._T):
            ...
            if self._using_tabu:
                ...
                for p in self._population:
                    ...
                    if post_tabu[-1].fitness >= self._max_fitness:
                        self._handle_optimal_converge(gen_start, post_tabu[-1])
                        break
            ...
            if self.best_chromosome.fitness >= self._max_fitness:
                self._handle_optimal_converge(gen_start, self.best_chromosome)
                break
            ...
            while len(next_generation) != len(self._population):
                ...
                if co1.fitness >= self._max_fitness or co2.fitness >= self._max_fitness:
                    self._handle_optimal_converge(
                        gen_start, co1 if co1.fitness >= self._max_fitness else co2
                    )
                    break

                co1, co2 = self._mutate(self._cop, co1, self._pm), self._mutate(
                    self._cop, co2, self._pm
                )
                if co1.fitness >= self._max_fitness or co2.fitness >= self._max_fitness:
                    self._handle_optimal_converge(
                        gen_start, co1 if co1.fitness >= self._max_fitness else co2
            ...
            self.time_hist.append(time.perf_counter() - gen_start)
            self._print_info(
                f"==> Generation {t} Completed in {self.time_hist[-1]:.2f} Seconds", 1
            )
    ...
    def _flip_tabu(self, cur: Chromosome) -> Chromosome:
        ...
        for _ in range(self._tabu_max_iter):
            count, best_move, best_fitness = 0, None, float("-inf")
            while count < self._tabu_max_explore:
                ...
                for gene in self._cop.domain:
                    ...
                    if move in tabu_list or current.data[ind] == gene:
                        continue
                    og_gene, og_fitness = current.data[ind], current.fitness
                    current.data[ind] = gene
                    self._cop.evaluate_fitness(current)
                    if current.fitness > og_fitness:
                        return current
                    else:
                        count += 1
                        ...
                        current.data[ind] = og_gene
                        current.fitness = og_fitness
            tabu_list.append(best_move)
            current.data[best_move[0]] = best_move[1]
    ...
    def _swap_tabu(self, cur: Chromosome) -> Chromosome:
        ...
        for _ in range(self._tabu_max_iter):
            ...
            for i, j in random_combination():
                if (
                    tuple(sorted([i, j])) in tabu_list
                    or current.data[i] == current.data[j]
                ):
                    continue
                og_fitness = current.fitness
                current.swap(i, j)
                self._cop.evaluate_fitness(current)
                if current.fitness > og_fitness:
                    return current
